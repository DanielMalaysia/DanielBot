    ...
    try {
      ...
      for (let name in global.plugins) {
        ...
        if ((usedPrefix = (match[0] || '')[0])) {
          ...
          if (xp > 200) m.reply('Dilarang cheat') // Hehehe
          else m.exp += xp
          if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
            this.reply(m.chat, `Limit anda telah habis`, m)
            continue // Limit habis
          }
          if (plugin.level > _user.level) {
            this.reply(m.chat, `diperlukan level ${plugin.level} untuk menggunakan perintah ini`, m)
...
global.dfail = (type, m, conn) => {
  let msg = {
    rowner: 'Perintah ini hanya dapat digunakan oleh Host Owner Bot',
    owner: 'Perintah ini hanya dapat digunakan oleh Owner Bot',
    mods: 'Perintah ini hanya dapat digunakan oleh Pengelola Bot',
    ...
    unreg: 'Silakan mendaftar untuk menggunakan perintah ini'
  }[type]
